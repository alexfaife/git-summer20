...
def setName():
    ...
    if choice != 'l' and choice != 'c':
        ...
        e.delete(0, END)
        top.set(
            'What would you like to name you character? Remember, they have ' + player.type + ' powers with ' + player.power + ' range abilities')
...
def gamePlay():
    ...
    top.set('''Your name is ''' + charName + ''' and you are a super hero with ''' + player.type +
            ''' powers. These powers include ''' + player.powers[0] + ', ' + player.powers[1] + ', ' + player.powers[
                2] +
            '''.
...
include ''' + powers + '''.''')
            ...
            powers = powers[6]
            top.set(
                '''Welcome to the Gobi Desert. The Desert has been reaching deathly temperatures because of the villain, ''' + villain +
                ''' and his powers include ''' + powers + '''.''')
...
on another mission. Would you like to play again or quit?''')
        ...
        top.set(
            'Nice! You\'ve hit ' + villain + ' with a power level of ' + str(level) + ' and their health is now ' + str(
                villainHealth) + '''.
They hit you back with an attack that deals ''' + str(villainAttack) + ''' damage. Your health is now ''' + str(
                player.health) + ''' 
